...
[
  ...
  const runCodeFix = useCallback(async () => {
    const missing = holdings.filter((h) => !h.code && (h.cat === "tokutei" || h.cat === "nisa_growth" || h.cat === "nisa_old" || h.cat === "tsumitate" || h.cat === "us"));
    if (!missing.length) return;
    setCodeFixing(true);
    try {
      const names = [...new Set(missing.map((h) => h.name))];
      const isUs = (n) => missing.find((h) => h.name === n)?.cat === "us";
      const prompt = `以下の証券名に対して、証券コードまたはティッカーシンボルを返してください。
日本株・ETF・投資信託: 4桁の数字コード（例: "7203"）
米国株: ティッカーシンボル（例: "AAPL"）
わからない場合はnullを返してください。
JSONのみ、マークダウン無し。形式: [{"n":"銘柄名","cd":"コード or null"}]
対象: ${JSON.stringify(names)}`;
      const resp = await fetch("https://api.anthropic.com/v1/messages", {
        method: "POST",
        headers: { "Content-Type": "application/json", "x-api-key": window._portfolioApiKey || localStorage.getItem("anthropic_api_key") || "", "anthropic-version": "2023-06-01", "anthropic-dangerous-direct-browser-access": "true" },
      ...
      const map = {};
      for (const m of matches) {
        try { const o = JSON.parse(m); if (o.n && o.cd) map[o.n] = o.cd; } catch {}
      }
      setHoldings((prev) => prev.map((h) => (!h.code && map[h.name]) ? { ...h, code: map[h.name] } : h));
    } catch (e) {
      console.error("コード補完失敗:", e);
    } finally {
      setCodeFixing(false);
    }
  }, [holdings]);
...
{"indices":[{"n":"日経平均","ytd":数値,"d1":数値},{"n":"TOPIX","ytd":数値,"d1":数値},{"n":"S&P500","ytd":数値,"d1":数値},{"n":"NASDAQ","ytd":数値,"d1":数値}],"sentiment":"上げ相場|下げ相場|横ばい","summary":"1文要約","sectors":[{"s":"セクター名","r":数値}]}`;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--text)", fontFamily: "system-ui, -apple-system, 'Segoe UI', sans-serif" }}>
      ...
      <div style={{ maxWidth: 880, margin: "0 auto", padding: "20px 16px 48px" }}>
        ...
        {tab === "holdings" && (
          <div>
            {/* コード補完ボタン */}
            {holdings.some((h) => !h.code && (h.cat === "tokutei" || h.cat === "nisa_growth" || h.cat === "nisa_old" || h.cat === "tsumitate" || h.cat === "us")) && (
              <div style={{ marginBottom: 14, padding: "10px 14px", background: "rgba(59,130,246,0.07)", border: "1px solid rgba(59,130,246,0.25)", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}>
                <div style={{ fontSize: 12, color: C.muted }}>
                  コード未取得の銘柄があります。AIで自動補完するとチャートが表示できます。
                </div>
                <button onClick={runCodeFix} disabled={codeFixing} style={{
                  padding: "7px 16px", background: codeFixing ? C.border : C.accent, border: "none",
                  borderRadius: 8, color: "#fff", fontSize: 12, fontWeight: 700, cursor: codeFixing ? "default" : "pointer", fontFamily: "inherit", whiteSpace: "nowrap",
                }}>{codeFixing ? "⟳ 補完中..." : "🔍 コードを自動補完"}</button>
              </div>
            )}
            {holdings.length === 0 ? (
